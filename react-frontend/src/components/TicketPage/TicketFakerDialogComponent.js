...
import ticketFakerFactory from "./ticketFakerFactory";
...
const TicketFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = ticketFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("ticket").create(elem));
...
export default connect(mapState, mapDispatch)(TicketFakerDialogComponent);
