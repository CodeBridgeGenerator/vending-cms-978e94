...
import seederData from "./hcStage2.seeder.json";
...
const HcStage2SeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("hcStage2").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(HcStage2SeederDialogComponent);
