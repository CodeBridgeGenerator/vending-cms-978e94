...
import hcStage2FakerFactory from "./hcStage2FakerFactory";
...
const HcStage2FakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    
                const [RefNoIds, setRefNoIds] = useState();
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $select: ["_id","RefNo"] } })
            .then((data) => {
              setRefNoIds(data.data);
            });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = hcStage2FakerFactory(fakerCount,RefNoIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("hcStage2").create(elem));
...
export default connect(mapState, mapDispatch)(HcStage2FakerDialogComponent);
