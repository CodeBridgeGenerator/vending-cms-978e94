...
import seederData from "./hCMasterForm.seeder.json";
...
const HCMasterFormSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("hCMasterForm").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(HCMasterFormSeederDialogComponent);
