...
import hCMasterFormFakerFactory from "./hCMasterFormFakerFactory";
...
const HCMasterFormFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = hCMasterFormFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("hCMasterForm").create(elem));
...
export default connect(mapState, mapDispatch)(HCMasterFormFakerDialogComponent);
