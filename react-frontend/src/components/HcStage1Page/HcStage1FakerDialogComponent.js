...
import hcStage1FakerFactory from "./hcStage1FakerFactory";
...
const HcStage1FakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = hcStage1FakerFactory(fakerCount,RefIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("hcStage1").create(elem));
...
export default connect(mapState, mapDispatch)(HcStage1FakerDialogComponent);
