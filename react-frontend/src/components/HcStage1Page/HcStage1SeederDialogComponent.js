...
import seederData from "./hcStage1.seeder.json";
...
const HcStage1SeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("hcStage1").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(HcStage1SeederDialogComponent);
