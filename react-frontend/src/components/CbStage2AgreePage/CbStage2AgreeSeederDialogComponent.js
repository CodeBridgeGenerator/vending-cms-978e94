...
import seederData from "./cbStage2Agree.seeder.json";
...
const CbStage2AgreeSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("cbStage2Agree").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(CbStage2AgreeSeederDialogComponent);
