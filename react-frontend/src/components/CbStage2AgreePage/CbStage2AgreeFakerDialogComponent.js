...
import cbStage2AgreeFakerFactory from "./cbStage2AgreeFakerFactory";
...
const CbStage2AgreeFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = cbStage2AgreeFakerFactory(fakerCount,RefIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("cbStage2Agree").create(elem));
...
export default connect(mapState, mapDispatch)(CbStage2AgreeFakerDialogComponent);
