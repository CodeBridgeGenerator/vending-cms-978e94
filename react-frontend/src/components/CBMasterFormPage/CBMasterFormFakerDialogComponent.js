...
import cBMasterFormFakerFactory from "./cBMasterFormFakerFactory";
...
const CBMasterFormFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = cBMasterFormFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("cBMasterForm").create(elem));
...
export default connect(mapState, mapDispatch)(CBMasterFormFakerDialogComponent);
