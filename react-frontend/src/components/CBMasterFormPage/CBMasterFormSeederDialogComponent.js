...
import seederData from "./cBMasterForm.seeder.json";
...
const CBMasterFormSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("cBMasterForm").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(CBMasterFormSeederDialogComponent);
