...
import hcStage1AgreeFakerFactory from "./hcStage1AgreeFakerFactory";
...
const HcStage1AgreeFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = hcStage1AgreeFakerFactory(fakerCount,RefIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("hcStage1Agree").create(elem));
...
export default connect(mapState, mapDispatch)(HcStage1AgreeFakerDialogComponent);
