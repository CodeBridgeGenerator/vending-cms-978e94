...
import seederData from "./hcStage1Agree.seeder.json";
...
const HcStage1AgreeSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("hcStage1Agree").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(HcStage1AgreeSeederDialogComponent);
