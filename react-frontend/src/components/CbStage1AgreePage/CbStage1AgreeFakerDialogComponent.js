...
import cbStage1AgreeFakerFactory from "./cbStage1AgreeFakerFactory";
...
const CbStage1AgreeFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = cbStage1AgreeFakerFactory(fakerCount,RefIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("cbStage1Agree").create(elem));
...
export default connect(mapState, mapDispatch)(CbStage1AgreeFakerDialogComponent);
