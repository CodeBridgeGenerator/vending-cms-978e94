...
import seederData from "./cbStage1Agree.seeder.json";
...
const CbStage1AgreeSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("cbStage1Agree").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(CbStage1AgreeSeederDialogComponent);
