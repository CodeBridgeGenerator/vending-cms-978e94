...
import cbStage2FakerFactory from "./cbStage2FakerFactory";
...
const CbStage2FakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = cbStage2FakerFactory(fakerCount,RefIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("cbStage2").create(elem));
...
export default connect(mapState, mapDispatch)(CbStage2FakerDialogComponent);
