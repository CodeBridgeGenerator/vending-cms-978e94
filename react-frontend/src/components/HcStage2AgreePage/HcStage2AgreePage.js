...
const HcStage2AgreePage = (props) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("hcStage2Agree").remove(data[selectedEntityIndex]?._id);
            ...
            console.log({ error });
            props.alert({ title: "HcStage2Agree", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service("hcStage2Agree").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/hcStage2Agree/${data._id}`);
    };
        ...
        <div className="mt-5"  style={{minHeight : 'calc(90vh - 9rem)'}}>
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">HcStage2Agree </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <Button label="add" icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="hcStage2Agree-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="hcStage2Agree-datatable">
                    <HcStage2AgreeDatatable items={data} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} />
                 </div>
            </div>
            <AreYouSureDialog header="Delete" body="Are you sure you want to delete this record?" show={showAreYouSureDialog} onHide={() => setShowAreYouSureDialog(false)} onYes={() => deleteRow()} />
            <HcStage2AgreeEditDialogComponent entity={data[selectedEntityIndex]} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <HcStage2AgreeCreateDialogComponent show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <HcStage2AgreeFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <HcStage2AgreeSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
