...
import seederData from "./hcStage2Agree.seeder.json";
...
const HcStage2AgreeSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("hcStage2Agree").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(HcStage2AgreeSeederDialogComponent);
