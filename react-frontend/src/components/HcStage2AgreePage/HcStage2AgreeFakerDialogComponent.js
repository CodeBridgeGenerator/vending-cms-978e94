...
import hcStage2AgreeFakerFactory from "./hcStage2AgreeFakerFactory";
...
const HcStage2AgreeFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = hcStage2AgreeFakerFactory(fakerCount,RefIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("hcStage2Agree").create(elem));
...
export default connect(mapState, mapDispatch)(HcStage2AgreeFakerDialogComponent);
