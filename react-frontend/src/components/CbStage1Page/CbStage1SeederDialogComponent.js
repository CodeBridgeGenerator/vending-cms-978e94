...
import seederData from "./cbStage1.seeder.json";
...
const CbStage1SeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("cbStage1").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(CbStage1SeederDialogComponent);
