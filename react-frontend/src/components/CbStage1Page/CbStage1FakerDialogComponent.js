...
import cbStage1FakerFactory from "./cbStage1FakerFactory";
...
const CbStage1FakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = cbStage1FakerFactory(fakerCount,RefIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("cbStage1").create(elem));
...
export default connect(mapState, mapDispatch)(CbStage1FakerDialogComponent);
